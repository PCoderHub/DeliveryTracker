...
def login():
    ...
    if isUserBlocked:
        logging.error('More than three incorrect tries by {username}')
        return jsonify({
            'error': 'User blocked! Try after ' + timeRemaining + ' minutes'
        }), 401
    if not user:
        logging.error('Unauthorised access')
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if payload['role'] != role:
                logging.warning('Unauthorised access')
...
def delivery_route():
    if request.method == 'POST':
        ...
        if re.match(regex, fromL) and re.match(regex, toL):
            ...
            logging.warning('input pattern does not match')
            return jsonify({
...
def onedelivery(id):
    ...
    if request.method == 'DELETE':
        ...
        logging.info('Delivery {id} deleted')  
        return jsonify({
...
def statusupdate(id):
    ...
    if stat in stats:
        ...
        logging.info('Delivery {id} updated')    
        return jsonify({
            'message': 'Delivery ' + id + ' is updated!'
            })
    else:
